...
# Снимем мультииндекс с колонок, объединим уровни в один уровень с удобными именами
df.columns = ['_'.join(col).strip() for col in df.columns.values]

# Теперь колонки будут: 'Price_SPY', 'Close_SPY', 'High_SPY', и т.д.
# Можно переименовать, если хочешь просто убрать тикер:
df.columns = [col.replace('_SPY', '') for col in df.columns]

# Сделаем индекс просто датой (если нужно)
df = df.reset_index('Ticker', drop=True)  # если 'Ticker' был в индексе
